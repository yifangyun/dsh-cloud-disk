...
const DEFAULT_CLOUD_DISK_TOOL_TIMEOUT_MS = 3e4;
...
const Config = Schema.object({
	...
	timeoutMs: Schema.number().default(DEFAULT_CLOUD_DISK_TOOL_TIMEOUT_MS),
	pageLimit: Schema.number().default(50)
});
/** Configured timeout and page bound must be positive integers. */
function assertPositiveInteger(name, value) {
	if (!Number.isInteger(value) || value < 1) throw new Error(`tool-cloud-disk: ${name} must be a positive integer`);
}
/** Pending-call presentation: a generic card titled by the target or query. */
function presentListCall(args) {
	return {
		card: "generic",
		title: args.parentId ?? "Cloud disk root",
		kind: "read",
		rawInput: args
	};
}
...
function apply(ctx, config) {
	...
	if (resolved.list) ctx.tools.register(defineTool({
		...
		parameters: {
			parentId: {
				type: "string",
				description: "The folder id to list; omit for the root directory."
			},
			cursor: {
				type: "string",
				description: "The nextCursor from a previous page to page on."
			},
			limit: {
				type: "number",
				description: `Maximum items to return (default ${String(resolved.pageLimit)}).`
			}
		},
		output: {
			schema: {
			...
			render: (_args, value) => [{
				type: "text",
				text: formatPage(value, "Cloud disk contents:")
			}],
			presentationMeta: (_args, value) => ({
				ids: value.nodes.map((node) => node.id),
				hasMore: value.nextCursor !== void 0
			})
		},
		...
		async execute(args, exec) {
			...
			return pageValue(await ctx.cloudDisk.list(request, exec.signal));
		},
		presentCall: presentListCall
	}));
	if (resolved.search) ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: formatPage(value, "Cloud disk results:")
			}],
			presentationMeta: (_args, value) => ({
				ids: value.nodes.map((node) => node.id),
				hasMore: value.nextCursor !== void 0
		...
		async execute(args, exec) {
			...
			return pageValue(await ctx.cloudDisk.search({
				query: args.query,
				...args.cursor !== void 0 ? { cursor: args.cursor } : {},
				limit: args.limit ?? resolved.pageLimit
			}, exec.signal));
...
//#endregion
export { Config, DEFAULT_CLOUD_DISK_TOOL_TIMEOUT_MS, DEFAULT_PAGE_LIMIT, apply, inject, name };
