...
这个可选 Profile Bundle 安装 Host 侧的 CloudDisk 服务、直连 360 API Provider 和应用侧边栏中的云盘页面；它不会向任何 Agent 默认增加工具。本仓库集成发行的 `dsh` 保留随包的 `cloud-disk` Preset，但独立 Bundle 不会为外部 `dsh` 安装 Preset，也不分发 Preset。Provider 通过 `dsh-credentials` 按引用读取 `CLOUD_DISK_API_KEY` 与 `CLOUD_DISK_SIGNING_SECRET`，Bundle 和 Preset 中均不保存密钥值。
...
- 独立 Bundle 只提供确定性的浏览界面。它不会安装用户 Preset，不会向通用 Session 增加 CloudDisk 工具，也不提供外部 CLI Preset 安装命令。
