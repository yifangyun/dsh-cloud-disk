...
const TOOL_NAMES = new Set([
    'file-list', 'file-search', 'make-dir', 'file-move', 'file-rename',
    'file-save', 'file-share', 'file-upload-stdio', 'get-download-url', 'user-info',
]);
const APPROVAL_NAMES = new Set([
    'make-dir', 'file-move', 'file-rename', 'file-save', 'file-share', 'file-upload-stdio',
]);
...
function toolParameters(inputSchema) {
    const candidate = typeof inputSchema === 'object' && inputSchema !== null && '$schema' in inputSchema
        ? (({ $schema: _schema, ...schema }) => schema)(inputSchema)
        : inputSchema;
...
async function connect(ctx, config) {
    ...
    const client = new Client({ name: 'dsh-cloud-disk-bundle', version: '0.1.1-rc.2' }, { capabilities: {} });
        ...
        for (const tool of listed.tools) {
            ...
            const definition = {
                ...
                async execute(args, exec) {
                    return await client.request({ method: 'tools/call', params: { name: tool.name, arguments: args } }, RAW_TOOL_RESULT_SCHEMA, { signal: exec.signal, timeout: config.toolCallTimeoutMs });
                },
            };
...
export async function apply(ctx, config) {
    ...
    ctx.on('internal/service', (service, value) => {
        if (service === 'credentials' && value !== undefined)
            refresh();
    }, { global: true });
    ctx.on('tools/pre-execute', (exec, next) => {
        if (APPROVAL_NAMES.has(exec.name.slice(`mcp__${SERVER_NAME}__`.length))) {
            return Promise.resolve({ kind: 'ask', reason: `Approve ${exec.name}` });
        }
        return next();
    });
    ctx.effect(() => () => {
        disposed = true;
        return updates.then(async () => { await stop?.(); });
    }, 'cloud-disk-mcp-host.connection');
}
//# sourceMappingURL=mcp-host.js.map