...
本文适用于 `0.1.1-rc.2.cloud.1`。运行机器必须有可用的 `dsh` 和 `pnpm`。
...
```sh
dsh plugin --profile web add @aicloud360/dsh-cloud-disk-bundle@0.1.1-rc.2.cloud.1
```

或从 GitHub 安装同一发布：

```sh
dsh plugin --profile web add github:yifangyun/dsh-cloud-disk#v0.1.1-rc.2.cloud.1
```
...
Bundle 只提供 Host 和页面。要让新 Agent 使用只读云盘工具，在 Desktop 的 Preset 广场安装并选择“云盘模式”。
...
以下命令构建五个包，将其作为本地 `file:` 依赖装入临时 Profile。CLI 验证会解析 npm 的官方 `@deepseek-ai/dsh@latest` 并使用该精确版本启动；Desktop 验证使用当前项目的 Desktop Host。不会向 npm 发布、写入 `~/.dsh` 或修改现有 Desktop 数据。关闭 CLI Web 或 Desktop 窗口后，脚本会删除临时目录。
...
该命令把五个公开命名包写入被 Git 忽略的 `artifacts/cloud-disk-local/`，其内部依赖指向相邻的本地包。它不是 npm 或 GitHub 发布物，不能用于插件中心的正式安装验证。为检查验证后的临时目录，可给启动命令加入 `--keep`。
